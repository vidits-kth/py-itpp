...
//  boost::python::scope().attr("QLLRmat") =  boost::python::object(boost::python::ptr(&itpp::QLLRmat));
  ...
  boost::python::class_<itpp::LLR_calc_unit>("LLR_calc_unit",
					     ...
					     " efficiency.",
					     boost::python::init<>("Constructor, using the default table resolution", boost::python::args("self")))

    .def(boost::python::init<short int, short int, short int>("Constructor, using a specific table resolution. See init_llr_tables() for more details on the parameters.", boost::python::args("self", "Dint1", "Dint2", "Dint3")))
